 ...
 * <p>
 * The Logger object internally uses the Java default
 * {@link java.util.logging.Logger} object and it's capabilities to log messages
 * with Handler objects.</p>
 * <p>
 * Logger objects may be obtained by calls of the {@code getLogger} factory
 * method. These will either create a new Logger or return the existing Logger
 * object with the desired name. The name is normally the hierarchical
 * dot-separated namespace of the package and class name.</p>
 * <p>
 * Example how to get a Logger object in the class {@code Test}:</p>
 * <blockquote><pre style="font-weight:bold">
 * public class Test { private static final Logger LOG =
 * Logger.getLogger(Test.class.getName());
 *
 * ... }
 * </pre></blockquote>
 * <!--<p>
 * It is important to note that the Logger returned by the {@code getLogger}
 * factory method may be garbage collected at any time if a strong reference to
 * the Logger is not kept.</p>-->
 * <p>
 ...
 * OS logs, etc.</p>
 * <p>
 * In contrast to Java's default {@link java.util.logging.Logger} object, all
 * Logger objects are automatically bound to one single "parent" Logger. This
 * ensures, that it is sufficient to add a Handler object one times to the
 * "parent" Logger.</p>
 * <p>
 ...
 *
 * public static void main (String[] args) { LOGP.addHandler(new
 * LogOutputStreamHandler(System.out)); ... }
 * </pre></blockquote>
 * <p>
 * <br>
 * <h3>Logger levels</h3>
 ...
 * then its effective level is inherited from its parent. By default the log
 * level of the "parent" Logger is set to {@code ALL}.</p>
 * <p>
 * For Logger objects created with the factory method
 * {@link #getLogger(String)}, the default logger level can be set with the
 * System Property <b>&lt;Logger-Name&gt;.Logger.Level</b>. This property can be
 * defined inside the Java program (using the method {@link System#setProperty(String, String)
 * }, for example in the static block of the Main-Class which is executed
 * first), or from outside with the option -D when starting the Java VM with the
 * command <b>java</b>.</p>
 * <p>
 * If this system property is not defined, the logger is configured with the
 * value of the System Property <b>logging.Logger.Level</b>. If this property is
 * not defined the logger level is configured to Level.INFO by default.</p>
 * <p>
 * For the package and class name, the wildcards * and ? can be used. In case of
 * a name conflict, a complete name without wildcards is preferred. If the name
 * is not found the value of the property logging.Logger.Level is choosen. If
 * this property is not defined the Level INFO is taken.</p>
 * <p>
 * Here is an example:</p>
 * <ul>
 * <li>the Logger object of the class {@code main.Main} is configured to
 * CONFIG</li>
 * <li>the Logger objects of all other classes in package {@code main.Main} are
 * configured to INFO</li>
 * <li>all other Logger objects will be configured to WARNING</li>
 * </ul>
 * <blockquote><pre style="font-weight:bold">
 * java -jar myProgram.jar \ -D"main.Main.Logger.Level=CONFIG" \
 * -D"main.*.Logger.Level=INFO" \ -D"*.Logger.Level=WARNING"
 * </pre></blockquote>
 * <p>
 * Instead of level names (SEVERITY, WARNING, ...) numbers can be used and will
 * be automatically translated to the matching predefined level or a new
 * instance of the class {@link java.util.logging.Level}.</p>
 * <p>
 * The log level can also be dynamically changed by calls on the
 * {@link #setLevel(Level)} method. The {@link java.util.logging.Level} class
 * supports a number of predefined Levels
 * ({@link java.util.logging.Level#SEVERE}, {@link java.util.logging.Level#WARNING},
 * ... , {@link java.util.logging.Level#FINEST}). These predefined values could
 * be enlarged by extending the class {@link java.util.logging.Level}.</p>
 * <p>
 * <br>
 * <h3>Logging methods</h3>
 * <p>
 * On each logging method call the Logger initially performs a cheap check of
 * the request level (DEBUG, SEVERE, WARNING, INFO, CONFIG, FINE, FINER, FINEST
 * or ALL) against the effective log level of the logger. If the request level
 * is lower than the log level, the logging call returns immediately.</p>
 * <p>
 * After passing this initial (cheap) test, the Logger will create a LogRecord
 * object to describe the logging message. It will then call a Filter (if
 * present) to do a more detailed check on whether the record should be
 * published. If that passes it will then publish the LogRecord to its Handlers
 * and to the "parent" Logger.</p>
 * <p>
 * The logging methods are grouped in the following categories:
 * <ul>
 * <li><b>log</b> methods; taking a individual log level and optionally some
 * parameters.
 * <li><b>debug</b> methods; these methods are used while development and should
 * be removed completely for released versions.
 ...
 * <li><b>all</b> methods; for highest verbosity of a program, only handled if
 * Logger is configured on level ALL which is the highest avaliable level.
 * </ul>
 * <p>
 * Each method group supports a various collection of overloaded methods:
 * <ul>
 * <li><b>no params</b>: LogRecord without any additional information (normally
 * not used)
 * <li><b>msg</b>: LogRecord with a message string
 * <li><b>msgFormat</b>: LogRecord with a message string which is created with a
 * {@link java.util.Formatter} (like {@code String.format} method)
 ...
 * <li><b>stackTraceDepth</b>: LogRecord with call Stack-Trace in the desired
 * depth
 ...
 * <blockquote><pre style="font-weight:bold">
 * LOG.info("Hello world"); LOG.info("Starting application version %s",
 * VERSION); LOG.warning(exception, "Error occurs when parsing the file);
 * LOG.warning(exception, "Error occurs when parsing the file '%s'", filename);
 * LOG.finest(bytearray); LOG.finest(bytearray, "data block of frame %d",
 * frameId); if (LOG.isFinestLoggable()) LOG.finest(new
 * LogRecordDataString(text)); ...
 * </pre></blockquote>
 * <p>
 * Control characters in messages are changed to a human readable format. For
 * example a line feed (value '\n'=10=0x0a) is changed to "&lt;0a&gt;".</p>
 * <p>
 * To ensure a special output behavior for special kind of data, a
 * {@link logging.LogRecordData} object can be used as parameter of the log
 * method. This class is abstract and the method
 * {@link logging.LogRecordData#print(PrintStream)} must be implemented to get
 * the desired output format.</p>
 * <p>
 * There are some concrete implementations of {@link logging.LogRecordData}
 * already available:
 * <ul>
 * <li>{@link logging.LogRecordDataString} ... for Strings with Line Feeds</li>
 * <li>{@link logging.LogRecordDataHexDump} ... for byte arrays in a hexdump
 * output format</li>
 * <li>{@link logging.LogRecordDataFormattedText} ... for text with underlines,
 * colors, ...</li>
 * </ul>
 * <br>
 * <h3>Logging filters</h3>
 * <p>
 * When logging a message, a cheap initial test checks the log level. If the
 * message is loggable the Logger will create a LogRecord object to describe the
 * logging message. Afterwards it will call a Filter to do a more detailed check
 * on whether the record should be published. The system property
 * <b>&lt;Logger-Name&gt;.Logger.Filter</b> can be used to configure a filter in
 * the getLogger factory method.</p>
 * <p>
 * Example to set a Filter for the logger of class main.Main:
 * <blockquote><pre style="font-weight:bold">
 * java -jar myProgram.jar -D"logging.Logger.Level=WARNING"
 * -D"main.Main.Logger.Filter=main.MainFilter"
 * </pre></blockquote>
 * <h3>Call location</h3>
 * <p>
 * If the {@code showLocation} attribute of the Logger object is {@code true},
 * the call location is automatically stored in the LogRecord object by using
 * the {@link logging.ExtendedLogRecord} class instead of the normal
 * {@link java.util.logging.LogRecord} class. In this case the location is
 * printed out by the Handler.<br>
 * This feature is very useful inside the IDE, because locations are shown as
 * hyperlink and can be selected easily.<br>
 * The {@code showLocation} attribute is true by default, but can be dynamically
 * changed by calls on the {@link #setShowLocation(boolean)} method.</p>
 * <p>
 * <br>
 * <h3>Publishing log records in background</h3>
 * <p>
 * Log records are processed by the Handler's publish method. This may need some
 * extra time and could delay normal program flow. If this additional delay is
 * not desired (because it may lead to program malfunction), it is suggested to
 * delegate the handler's publish method to a background thread. This can easily
 * be done by using the class {@link logging.LogBackgroundHandler}. In this case
 * ensure that the {@link logging.LogBackgroundHandler} object is proper closed
 * on the end of the program, and time is given to flush out the queued records.
 * This can be done by call of the methods
 * {@link logging.LogBackgroundHandler#close()} and/or
 * {@link logging.LogBackgroundHandler#awaitTermination(long, TimeUnit)}.</p>
 * <p>
 * Example of binding a Background-Thread-Handler to the "parent" Logger
 * object:</p>
 * <blockquote><pre style="font-weight:bold">
 * private static final Logger LOGP = Logger.getParentLogger();
 *
 * public static void main (String[] args) { try { LOGP.addHandler(new
 * LogBackgroundHandler(new LogOutputStreamHandler(System.out))); ... } finally
 * {
 * ((LogBackgroundHandler)Logger.getParentLogger().getHandlers()[0]).awaitTermination(1,
 * TimeUnit.SECONDS); } }
 * </pre></blockquote>
 * <h3>System properties</h3>
 * <p>
 * Special system properties are available in order to help doing a correct
 ...
 * <td>Print logging configuration (Levels, Filters, Handlers) on
 * {@code System.out}</td>
 * </tr>
 * <tr>
 ...
 * <td>Log Level for all class Logger objects, if nothing defined in other
 * way.</td>
 * </tr>
 * <tr>
 * <td>{@code logging.Logger.ParentLevel}</td>
 * <td>Log Level for the "parent" logger class. Use this property if the default
 * value (Level ALL) is not desired.</td>
 * </tr>
 * <tr>
 * <td>{@code <complete class-name>.Logger.Level}</td>
 * <td>Log Level for the class's logger.<br>
 * &lt;complete class-name&gt; is the hierarchical dot-separated namespace of
 * the package and class name (wildcards * and ? are possible).</td>
 * </tr>
 * <tr>
 ...
 * <td>Define this property if LogOutputStreamHandler should print out class
 * name and hashcode of any log record.</td>
 * </tr>
 * <tr>
 * <td style="padding-right:15px">{@code logging.LogOutputStreamHandler.colorize}</td>
 * <td>Define this property if LogOutputStreamHandler should print out colorized
 * log level (red for severe, yellow for warning, green for info, cyan for
 * debug). For colorization ANSI escape sequences are used. This feature is
 * enabled by default. It can be disabled by defining the value "false" for this
 * property.
 * </td>
 * </tr>
 * </table>
...
public class Logger {
     ...
     * After shutdown getLogger() and getParentLogger() will not create new
     * Logger objects. Handlers publish method will not accept any new records.
     */
     ...
     * waiting for prcessing pending records.
     *
     * <p>
     * This method does not wait for actively executing tasks to terminate. Use
     * {@link #awaitTermination awaitTermination} to do that.
     *
     * @return list of handlers which are not terminated proper or null if
     * shutdownNow was already executed
     */
    ...
    /**
     * Shutdown Logger System and wait until all records (in all Handler
     * objects) are processed. A call to this method blocks the caller until no
     * records are pending, timeout occurs or logger threads are interrupted.
     * The output stream will be closed afterwards. A seperate call to the
        ...
        switch (level.toUpperCase()) {
            ...
            default:
                ...
                if (l == Level.ALL.intValue()) {
                    return Level.ALL;
                } else if (l == Level.FINEST.intValue()) {
                    return Level.FINEST;
                    ...
                    return Level.SEVERE;
                } else if (l == DEBUG.intValue()) {
                    return DEBUG;
                } else if (l == Level.OFF.intValue()) {
                    return Level.OFF;
                } else {
    ...
    /**
     * Set the log level for the debug method calls. By default the level
     * {@link #DEBUG} is used, which value is (Level.OFF.intvalue() - 1)
     *
    ...
    /**
     * Log a empty log record for debugging purposes while development. The log
     * level is {@link #DEBUG} by default, but can be changed by
    ...
    /**
     * Log message for debugging purposes while development. The log level is
     * {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log Throwable for debugging purposes while development. The log level is
     * {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log Throwable and message for debugging purposes while development. The
     * log level is {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
     * @param th Throwable object (Exception, Error, ...)
    ...
    /**
     * Log a LogRecordData object for debugging purposes while development. The
     * log level is {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log a LogRecordData object and a message string for debugging purposes
     * while development. The log level is {@link #DEBUG} by default, but can be
    ...
    /**
     * Log a byte array for debugging purposes while development. The log level
     * is {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log a byte array and a message string for debugging purposes while
     * development. The log level is {@link #DEBUG} by default, but can be
     * changed by {@link #setDebugLevel(Level)}. The call should be removed for
     * released software versions.
     *
     * @param data byte array which is transferred to an
    ...
    /**
     * Log a Throwable object and a LogDataRecord object for debugging purposes
     * while development. The log level is {@link #DEBUG} by default, but can be
     * changed by {@link #setDebugLevel(Level)}. The call should be removed for
     * released software versions.
     *
    ...
    /**
     * Log a Throwable object, a LogDataRecord object and a messsage string for
     * debugging purposes while development. The log level is {@link #DEBUG} by
    ...
    /**
     * Log call stack with limited depth for debugging purposes while
     * development. The log level is {@link #DEBUG} by default, but can be
     * changed by {@link #setDebugLevel(Level)}. The call should be removed for
     * released software versions.
     *
    ...
    /**
     * Log call stack with limited depth and message string for debugging
     * purposes while development. The log level is {@link #DEBUG} by default,
     * but can be changed by {@link #setDebugLevel(Level)}. The call should be
    ...
    /**
     * Log call stack with limited depth and Throwable object for debugging
     * purposes while development. The log level is {@link #DEBUG} by default,
     * but can be changed by {@link #setDebugLevel(Level)}. The call should be
     * removed for released software versions.
     *
    ...
    /**
     * Log call stack with limited depth, a Throwable object and a message
     * string for debugging purposes while development. The log level is
     * {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log call stack with limited depth and a LogDataRecord object for
     * debugging purposes while development. The log level is {@link #DEBUG} by
     * default, but can be changed by {@link #setDebugLevel(Level)}. The call
     * should be removed for released software versions.
     *
    ...
    /**
     * Log call stack with limited depth, a LogDataRecord object and a message
     * string for debugging purposes while development. The log level is
     * {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
    ...
    /**
     * Log call stack with limited depth, a LogDataRecord object and a Throwable
     * object for debugging purposes while development. The log level is
     * {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log call stack with limited depth, a LogDataRecord object, a Throwable
     * object and a message string for debugging purposes while development. The
     * log level is {@link #DEBUG} by default, but can be changed by
     * {@link #setDebugLevel(Level)}. The call should be removed for released
     * software versions.
     *
    ...
    /**
     * Log a Throwable object and message string with
     * {@link java.util.logging.Level#ALL}.
     *
     * @param th the Throwable object
     * @param msg the message string
     */
    ...
    /**
     * Log LogRecordData and message string with
     * {@link java.util.logging.Level#ALL}
     *
     * @param data the LogRecordData object
     * @param msg the message string
    ...
    /**
     * Log byte array and message string with
     * {@link java.util.logging.Level#ALL}
     *
     * @param data byte array which is transferred to an
     * {@link logging.LogRecordDataHexDump} object
    ...
    /**
     * Log Throwable and LogRecordData object with
     * {@link java.util.logging.Level#ALL}
     *
     * @param th the Throwable associated with this
    ...
    /**
     * Log call stack with limited depth and message string with
     * {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth and Throwable with
     * {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth, Throwable and message string with
     * {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth, and LogRecordData with
     * {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth, LogRecordData and message string with
     * {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth, Throwable and LogRecordData with
     * {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#ALL}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#FINEST} would
     * actually be logged by this logger. This check is based on the Loggers
    ...
    /**
     * Log call stack with limited depth and
     * {@link java.util.logging.Level#FINEST}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#FINEST}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#FINER} would
     * actually be logged by this logger. This check is based on the Loggers
     * effective level, which may be inherited from its parent.
     *
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#FINER}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#FINE} would
     * actually be logged by this logger. This check is based on the Loggers
     * effective level, which may be inherited from its parent.
     *
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#FINE}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#CONFIG} would
     * actually be logged by this logger. This check is based on the Loggers
     * effective level, which may be inherited from its parent.
     *
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#CONFIG}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#INFO} would
     * actually be logged by this logger. This check is based on the Loggers
     * effective level, which may be inherited from its parent.
     *
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#INFO}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#WARNING} would
     * actually be logged by this logger. This check is based on the Loggers
     * effective level, which may be inherited from its parent.
     *
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#WARNING}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * Check if a message with {@link java.util.logging.Level#SEVERE} would
     * actually be logged by this logger. This check is based on the Loggers
     * effective level, which may be inherited from its parent.
     *
    ...
    /**
     * Log call stack with limited depth, Throwable, LogRecordData and message
     * string with {@link java.util.logging.Level#SEVERE}
     *
     * @param stackTraceDepth the depth of call stack trace which should be
     * published
     * @param th the Throwable associated with this
    ...
    // *************************************************************************
    /**
     * This class extends {@link java.util.logging.Level} and is used for Level
     * {@link #DEBUG}
     */
